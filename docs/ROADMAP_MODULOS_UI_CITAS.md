...
### Ajuste complementario: visibilidad antropométrica por perfil

- La columna **% Grasa** del Historial de consultas se oculta para Medicina general y Odontología.
- Nutrición conserva el encabezado y sus valores; no se eliminan ni modifican datos persistidos.
- La simplificación futura de Consultas clínicas queda definida en [ROADMAP_AGENDA_Y_CONSULTAS.md](ROADMAP_AGENDA_Y_CONSULTAS.md).

### Fase 1.19: Agenda operativa — completada en 1.8.0

- El sidebar abre `/agenda`; el Dashboard conserva únicamente el resumen del día.
- Vistas Día/Semana, periodo navegable, selector de fecha y conteos de estados.
- Alta contextual y reagenda reutilizan búsqueda, disponibilidad y bloqueo existentes.
- Reagenda conserva el ID y excluye sólo el espacio de la cita editada.
- Atendida, No Asistió y Cancelada son terminales; el servidor bloquea cierre futuro, reapertura y cancelación sin motivo.
- Recepción gestiona citas sin recibir motivo clínico ni inicio de consulta.
...
### Fase 1.21: contraste de pendientes y detalle progresivo — completada en 1.9.1

- Los nombres, estados secundarios y mensajes vacíos de **Pendientes de atención** reciben colores explícitos para el tema oscuro.
- Reposo, hover y foco visible cumplen una relación de contraste mínima de 4.5:1 según pruebas automatizadas.
- El detalle del paciente mantiene siempre los datos principales, el último pago y la siguiente cita.
- Correo, ocupación, dirección y contacto de emergencia sólo aparecen cuando fueron capturados; la ausencia conjunta se resume en un solo estado con acceso a edición.
- No se ocultan ausencias clínicamente relevantes, no se eliminan campos del alta/edición y no cambia el modelo de datos.
...
| UI-DASH-07 | Pendientes oscuros | Texto principal/secundario, hover y foco conservan contraste AA |
| UI-PAT-01 | Complementarios vacíos | Un solo estado accionable; datos principales/operativos conservados |
| UI-PAT-02 | Complementarios parciales | Sólo aparecen los campos opcionales realmente capturados |
