...
La acción **Agendar cita** del KPI abre una vista dedicada de cuatro pasos: paciente, fecha, horario y motivo. Un buscador filtra el selector nativo de pacientes sin construir HTML con contenido del usuario. El calendario muestra 21 días y el número de bloques libres; otra fecha puede capturarse con un control nativo. Los horarios se recuperan mediante `fetch`, se crean con APIs DOM seguras y diferencian texto/estado además del color. Si falla la consulta, la confirmación permanece bloqueada. El formulario resume paciente, expediente, fecha y hora antes de enviarse. Esta vista no aparece en el sidebar y no reemplaza el modal existente del detalle.
