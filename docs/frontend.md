...
Administración y Recepción disponen del enlace **Pagos** bajo Gestión. La pantalla global inicia con el día actual, presenta cuatro indicadores, desglose por método, filtros y una tabla paginada. La búsqueda por nombre completo funciona aunque sus términos estén almacenados en campos separados, escritos sin acentos, con otras mayúsculas o como fragmentos. Sólo Administración ve el control de cancelación; éste exige motivo y una confirmación visual con folio. El aviso indica que el pago seguirá en el historial, quedará **Cancelado**, dejará de contar en los totales y no podrá reactivarse; ofrece **Volver** y **Sí, cancelar pago**. Existe una confirmación nativa si el componente visual no está disponible. El formulario se despliega dentro del flujo de la tabla y después conserva el renglón visible y resaltado. `payments.css` define la presentación clara, oscura y adaptable del módulo.
...
El favicon institucional se declara tanto en el shell como en la vista independiente de receta. Su URL incluye la versión de la aplicación y la ruta de compatibilidad exige revalidación, evitando que el navegador conserve un icono anterior. La búsqueda global ejecuta la búsqueda existente de pacientes por nombre, teléfono o correo y comparte la normalización parcial sin mayúsculas ni acentos. La sede muestra el único consultorio local configurado y explica que la administración multi-sede aún no existe. Notificaciones muestra un estado vacío real. El tema claro/oscuro se guarda en `localStorage` bajo `sgpn-theme`; las vistas impresas siempre fuerzan fondo claro. En oscuro, las tarjetas usan superficies azul petróleo, los bordes decorativos se vuelven transparentes o de baja opacidad, los campos conservan foco visible sin mantener contornos agresivos, los enlaces heredan la paleta y los botones deshabilitados no conservan el gris nativo del navegador. Las variantes Tailwind `hover:bg-gray-*`, `hover:bg-teal-*`, `hover:bg-emerald-*`, `hover:bg-amber-*` y `hover:bg-red-*` tienen equivalentes oscuros explícitos en `shell.css`, por lo que una fila o botón no cambia a blanco mientras conserva texto claro.

En edición de usuarios, la propia cuenta de Administración muestra rol y estado protegidos. El perfil clínico sigue disponible para que un administrador también atienda pacientes sin perder acceso. Los cambios de rol/estado hechos por otra cuenta cierran las sesiones anteriores del usuario afectado.
